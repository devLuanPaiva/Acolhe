...
export default class Caregiver {
  ...
  async addDiary(value: number, userId: number): Promise<void> {
    ...
    this.notifyCaregiver(
      `Um novo diário foi adicionado ao cuidador com ID ${userId}.`
    );
  }
  ...
  async getCaregiverByDiary(
    ...
    if (!caregivers || caregivers.length === 0) {
      throw new Error(
        "Nenhum cuidador encontrado dentro do intervalo de diários especificado."
      );
    }
  ...
  async evaluateCaregiver(value: string, userId: number): Promise<void> {
    ...
    await this.repo.evaluateCaregiver(value, userId);
  }
